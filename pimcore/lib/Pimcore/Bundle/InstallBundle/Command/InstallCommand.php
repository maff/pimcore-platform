...
class InstallCommand extends Command
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->getOptions() as $name => $config) {
            if (!$this->installedNeedsOption($config)) {
                continue;
            }
    ...
    private function installedNeedsOption(array $config): bool
    {
        if ('profile' === $config['group'] && !$this->installer->needsProfile()) {
            return false;
        }

        if ('db_credentials' === $config['group'] && !$this->installer->needsDbCredentials()) {
            return false;
        }
